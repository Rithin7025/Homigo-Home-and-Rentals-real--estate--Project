...
export const listUsers = async (req, res) => {
  if (req.role !== "admin") {
    return res
      .status(403)
      .json({ message: "Access Denied. Insufficient privilages" });
  }
...
export const getUnverifiedListings = async (req, res) => {
  ...
  try {
    const getUnverifiedListings = await Listing.find({
      isVerified: false,
    }).sort({ createdAt: -1 });
...
export const verifyListing = async (req, res) => {
  ...
  try {
    ...
    const updateListing = await Listing.findByIdAndUpdate(objectId, {
      isVerified: true,
    });
