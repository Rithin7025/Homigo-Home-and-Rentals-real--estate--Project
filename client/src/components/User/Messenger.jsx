...
const handleConversationClick = async(clickedConversation) => {
  ...
  const ownerId = clickedConversation.members.find((user)=> user !==  currentUser._id)
  ...
  useEffect(()=>{
    ...
    const fetchConversations = async()=>{
      ...
      console.log('before fecthing conver')
      const res = await axios.get(`/api/conversation/getUserConversation/${currentUser._id}`);
      setConversation(res.data)
      
      //setting the message with the owner if there is previous chat
      //api to find out the conversation id
      const conversationId = await axios.get('/api/conversation/getConversationId',{params : {senderId:currentUser._id, receiverId : ownerId}})
      ...
      if(conversationId.data._id){
        const res = await axios.get(`/api/message/getUserMessages/${currentChat?._id}`)
  ...
  useEffect(()=>{
   ...
   const handleSubmit = async (e) => {
    ...
    //check if there  
    if(!currentChat){
      const res = await axios.post('/api/conversation/newConversation',{senderId : currentUser._id,receiverId : ownerId})
      console.log(res)
      if(res){
       ...
       <div className='flex flex-row h-full'>
        ...
        {/**left div to display chats(contacts) */}
         <div className='w-3/5 p-3 border-r-2 bg-neutral-100 h-[555px]  border-slate-400 overflow-scroll overflow-x-hidden'>
           <div className="">
            <p className='text-xl font-semibold p-2 flex gap-2 items-center'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
</svg>Chats
</p>
            <input type="text" className='p-2 mb-2 mt-2 rounded-lg  right w-full focus:outline-none border-slate-800  border-b-[1px]' placeholder='search users'/>
         ...
         <div className='w-full'>
           ...
           <div className='flex items-center ml-5 p-3 gap-4'>
            <img className='h-10 w-10 rounded-full object-cover' src={owner?.avatar} alt="" />
                
            <div className='flex flex-col'>
            <p className='text-lg font-semibold text-white'>{owner?.userName}</p>
        ...
        <div className='w-full h-[500px]'>
         {
          message.map((mesg)=> (
            <div ref={scrollRef}>
              ...
              <Message key={mesg.conversationId} message={mesg} isOwn={mesg.sender === currentUser._id }/>
            </div>
           ...
           <textarea onChange={(e)=>setNewMessage(e.target.value)}  value={newMessage} id="text" cols="30" className=' w-full rounded-lg h-12 p-3 focus:outline-none' placeholder='Message' rows="10"></textarea>
           <button type='button' className='p-3 bg-blue-700 rounded-lg text-white font-semibold mr-4 hover:bg-blue-500' onClick={handleSubmit}>send</button>
            </div>
         </div>
