...
export default function CreateListing() {
  ...
  const [docsUploadErrors, setDocsUploadErrors] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if(formData.regularPrice < formData.discountPrice){
        return setFormError(`Discounted price cannot be lower than regular price`)
      }

      if (formData.imageUrls.length < 1) {
        return setImageUploadErrors("please add images to continue");
      }
      if (formData.docsUrls.length < 1) {
        return setDocsUploadErrors("please add Document images to continue");
      }
      setSpinnerLoad(true);
      setFormError(false);
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-6">
        ...
        >
          ...
          <p className="text-blue-700 font-semibold">
            Upload any 3 Legal documents of your property here{" "}
          </p>
          ...
          <div className="flex items-center">
            ...
            {docsSubmited && <p className="text-green-700">{docsSubmited}</p>}
          </div>
          {/**The Map component */}
          <div>
            <Leaflet
